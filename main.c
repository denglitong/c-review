...
// 都是允许的，但是双引号和尖括号是有区别的：
//  #include <> 表示引入的文件在 INCLUDE_DIR 中去查找
//      （INCLUDE_DIR
//      是由系统的环境变量进行设置的，一般为系统头文件的默认存放目录，如
//      /usr/include 目录下）
 ...
 * C 语句 = C 表达式 + ;
 *
 * C 函数不能返回数组，也不支持数组赋值，更多的原因是 C
 * 语言本身的特点需要贴合硬件，C 语句需要在几个机器指令内完成，所以
 * 不支持成百上千的数组的赋值操作；
 * 结构体 struct 是后来才加入
 * C语言的，一开始也是不支持结构体赋值的（结构体可包含多个字段犹如数组），但是因为结构体通常
 * 都比较小，所以做了一个权衡，在 C
 * 语言中支持结构体赋值，也可以在函数中返回结构体
 *
 ...
 * 函数名:
 * 函数名本身也有值，它代表了该函数的入口地址，使用指针调用函数时将用到此功能
 * 函数的定义只能有一次，而函数的声明可以有多次
 *
 * 变量是对程序中所占用空间的一种抽象定义，保存变量当前值的存储单元有 2
 * 类：内存、CPU 的寄存器。 变量的存储类型关系到变量的存储位置，C 语言中定义了 4
 * 种属性： 自动变量（auto）、 外部变量（extern），即全局变量、
 *  静态变量（static）：变量存储于内存的全局静态区，仅会初始化一次
 *  寄存器变量（register）：
 *      只有局部自动变量和形参可以作为寄存器变量；
 *      寄存器数量有限，寄存器变量不能随便定义；
 *      不能对寄存器变量进行取址操作；
 *      寄存器变量是为了提高运算效率，编译器有时候会把 register 自动忽略掉，即
 * register 只是一个建议，
 ...
 *
 * 指针: 类型说明符
 * *变量名，类型说明符表示该指针变量所指向的变量为何种数据类型，C 还提供使用 *
 * 获取地址上对应的值
 *
 * 地址是逻辑内存空间中的一个编号，内存中的计量单位为 1 个字节(byte)，即
 * 8比特(bit)，内存中的每个字节都有对应的一个编号
 *
 ...
 *
 * 某个类型的指针 p 是个变量，可以改变 p 使它指向不同的地址，但不能改变 p
 * 所指向的【常量】的值， 和 数组指针
 * 有着本质的区别，数组指针里面，指针指向的都是变量，所以可以通过数组指针修改某个下标的值。
 *
...
// scanf()
// 输入格式函数：按照格式字符串的格式，从键盘上把数据输入到指定的变量之中 1)
// 格式说明符中，可指定数据的宽度，但不能指定数据的精度 2) 输入 long
// 类型数据必须使用 %ld, 输入 double 类型数据必须使用 %lf 或 %le 3)
// 附加格式说明符 "*"
// 使对应的输入数据不赋给相应的变量(即使函数跳过相应的输入项目) %s
// 把输入解释成一个字符串：输入的内容以第一个非空白字符作为开始，并且包含直到下一个空白字符的全部字符
// scanf() 函数返回成功读入的项目的个数，如果没有成功读取到输入的项目时返回
// 0，遇到 EOF 时返回 EOF 值，一般是 -1
void scanf_exer() {
  int num;
...
void string_classification() {
  ...
  /**
   * ssize_t getline(
   *      // 指向存放该行字符的指针，如果是 NULL 则由系统帮助
   * malloc，需要在使用完成后 free 释放 char **lineptr,
...
void unary_quadratic_equation() {
  ...
  if (criterion > 0.0f) {
    printf("x1=%.3f x2=%.3f", -b + sqrt(criterion) / (2 * a),
           -b - sqrt(criterion) / (2 * a));
...
void show_pointer() {
  ...
  // 字符指针 str 是一个变量，可以改变 str 使它指向不同的字符串，但不能改变 str
  // 所指向的常量区的内容 str
  // 字符指针指向了一个常量字符串的首地址，常量字符串是不可以被更改的
  ...
  // 这个定义其实是将 2 这个值赋给了 p7 这个变量，即 p7 对应的地址为
  // 2，所以它的表现为 undefined
  int *p7 = {2, 1, 3};
...
void show_struct() {
  ...
  // The = operator cannot be used to copy the contents of one array to the
  // other, you must use a library function like strcpy or strcat for strings,
...
/**
 * C 语言的 union
 * 允许几种不同类型的变量存放到同一段内存单元中，union变量和成员变量的内存地址都是同一个地址；
...
void show_union() {
  ...
  // 不能把 union 当做函数参数，也不能是函数的返回体，但可以使用指向 union
  // 的指针 union 可以出现在 struct 中，也可以定义 union array； struct
  // 也可以出现在 union 中，array也可以出现在 union 中；
  union ABC abc;
  abc.a = 1;
  abc.b = 'b';
  abc.c = 3.14f;
  ...
  printf(
      "c:%.2f\n",
      abc.c);  // 2.13
               // 由于指向的地址相同，且成员定义的类型相同，所以指向最新的赋值成员
  printf("d:%.2f\n", abc.d);  // 2.13
}
...
void student_score_summary() {
  ...
  for (int i = 0; i < n; ++i) {
    // 在堆上分配内存，然后被 Student
    // 的引用带出去了，内存块持续存在，只是指向内存块的指针从 id,name 变为了
    // Student{id,name}； 这里的 id,name 是 for
    // 循环体在栈上分配的临时变量，如果不用 malloc
    // 动态分配，那么由于循环体的局部性，下次循环时上次循环体内定义的
    // 临时变量就要失效，这里编译器为了加速，会复用第一次循环体定义的临时变量，所以会看到
    // id,name 在循环体内被复用同一个地址的现象。
    char *id = malloc(256);
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (students[i].score1 + students[i].score2 + students[i].score3 >
        largest_sum) {
      largest_sum =
  ...
  printf("%s %s %d %d %d\n", students[largest_idx].id,
         students[largest_idx].name, students[largest_idx].score1,
         students[largest_idx].score2, students[largest_idx].score3);

  // malloc - free pair
  // C 为了极限性能不会加很多CPU
  // 指令的读写屏障去做一些自动化的内存管理功能，不像 Java/Go 的运行时
  for (int i = 0; i < n; ++i) {
    free(students[i].id);
...
void file_io() {
  ...
  // strlen 统计数组的个数，遇到 \0 或者 NULL 结束，统计的结果不包括 \0 或者
  // NULL 自身
 ...
 * 但它们并不是 C
 * 语言本身的组成部分，不能直接对他们进行编译，必须在编译之前，对这些指令进行"预处理"。
 * C 语言提供的预处理功能：
 * 1.宏定义
 *      宏定义允许在 C
 * 源程序中用一个标识符来标识一个字符串，这个标识符称为"宏名"；
 ...
 *  #define ADD(x) ((x)+(x))
 *  这样传入的 x
 * 的运算优先级就是确定的了，不会有字符串替换之后引起的优先级的逻辑问题。
 */
void show_define() {
  printf("%.6f\n", _PI);
  printf("%d\n", M(3));
 ...
 * __LINE__ 和 __FILE__
 * 都是编译程序中预定义的标识符，表示当前编译源文件的文件名和当前正在被编译的行
...
void left_shift_then_right_shift() {
  ...
  // C/C++ 中逻辑右移和算术右移共享同一个运算符
  // >>，编辑器决定使用逻辑右移还是算术右移，根据的是运算数的类型
  // 如果运算数类型是 unsigned 无符号型 则采用逻辑右移，
  // 如果运算符类型是 signed 有符号型 则采用算术右移；
  // 对于 signed 类型的数据，如果需要使用算数右移，或者 unsigned
  // 类型的数据需要使用逻辑右移，都需要进行 类型转换
  a = a >> (bits_len - 4);
  printf("%s\n", TO_BASE(a, 2));
 ...
 *  2.取指定位
 *      在某些存储场景下，"第 1~3 位表示xxx"，我们需要取出
 * 1~3位，则可以让原数值与数字7进行按位与运算，
 *      得到的结果即为原数值的1~3位的值；
 *  3.判断奇偶
 *      数字的奇偶取决于二进制位的最低一位是 1 还是 0，因此只需要与 1
 * 按位与运算，判断是 1 是 0 即可得知奇偶
 */
void show_and() {
  int a = 3 % 5;
  printf("%d\n", a);
...
/**
 * 按位或 | 运算符的作用：
 * 如"某个数字的第 7 位"表示开关，原先是 0，需要改为 1
 * 的状态，即可以将这个数字与 64 (2^6) 按位或， 即可得到第 7 位为
 * 1，其余位的值依旧不变；
 */
void show_or() {
  int a = 10, b = 5;
 ...
 *  1.指定位数的翻转：如果对某个数字的低 4 位进行翻转，则可以将这个数字与
 * 15（0000 1111）进行按位异或运算， 即可以将元数字的第四位进行翻转，其余位不变
 *  2.与 0 异或还是原值：
 *      (0000 1111) 与 0 异或，1 还是 1，,0还是 0，所以位不变
 *  3.交换两个数字：利用了 xor 两次得到原数字的原理，即 (a ^ b ) ^ b = a ^ (b ^
 * b) =  a ^ 0 = a int a, b; a = a ^ b; b = b ^ a; a = a ^ b;
 *    交换两个数字的第二种方法（可能溢出）：
 *      a = a + b;
 *      b = a - b;
 *      a = a - b;
...
void show_stack_args_order() {
  ...
  *(ptr++) += 100;
  ...
  //  occurs in any of the orderings.84).
  printf("%d %d\n", *ptr, *(++ptr));
}
 ...
 * 输出：1 为小端模式（数据低位存储在内存的低地址），0
 * 为大端模式（数据低位存储于内存的高地址） Mac 上述输出 1，为小端模式
 */
void show_int_storage_endian() {
  char aChar;
  int aInt;
...
// so the value of sizeof is calculated in the compile time, not in the
// execution time
void show_sizeof_keyword() {
  int i;
  i = 10;
