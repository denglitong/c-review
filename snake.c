...
// Follow: https://blog.csdn.net/charloy/article/details/106737038
//
#include "snake.h"
...
void gotoxy(int col, int row) { printf("%c[%d;%dH", ESC, row, col); }
...
void cleanUpKeyBoard() { tcsetattr(0, TCSAFLUSH, &oldTerminalSettings); }
...
void isKill(int *life, int snakeLen) {
  ...
  // 不管多少蛇身位于边界，一次循环只减一分
  for (int i = 0; i < snakeLen; ++i) {
    if (snake[i].x == 1 || snake[i].x == COL || snake[i].y == STARTING_ROW ||
        snake[i].y == ROW) {
      *life -= 1;
      gotoxy(LIFE_COL, LIFE_ROW);
      printf("\e[32mlife: %3d\e[0m", *life);
